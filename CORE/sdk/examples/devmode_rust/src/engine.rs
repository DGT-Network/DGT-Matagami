...
impl DevmodeService {
	...
	debug!("Finalizing block: self.service.finalize_block");
        ...
        while let Err(Error::BlockNotReady) = block_id {
            ...
            sleep(time::Duration::from_secs(1));
	    debug!("Finalizing block: self.service.finalize_block AGAIN");
            block_id = self.service.finalize_block(consensus.clone());
        }
        self.log_guard.not_ready_to_finalize = false;
	debug!("Finalizing block: Failed to finalize block");
...
impl Engine for DevmodeEngine {
	...
	debug!("After service.initialize_block");
        ...
        loop {
            ...
            if !published_at_height && time::Instant::now().duration_since(start) > wait_time {
                ...
                info!("Published -- make broadcast_published_block");

...
from smart_bgt.client_cli.workload.workload_generator import WorkloadGenerator
from smart_bgt.client_cli.workload.sawtooth_workload import Workload
from smart_bgt.client_cli.create_batch import create_smart_bgt_transaction
from smart_bgt.client_cli.create_batch import create_batch
from smart_bgt.client_cli.exceptions import SmartBgtCliException
from sawtooth_sdk.protobuf import batch_pb2

LOGGER = logging.getLogger(__name__)

SmartBgtState = namedtuple('SmartBgtState', ['name', 'url', 'value'])
...
class SmartBgtWorkload(Workload):
    """
    This workload is for the Sawtooth Smart Bgt transaction family.  In
    ...
    def __init__(self, delegate, args):
        super(SmartBgtWorkload, self).__init__(delegate, args)
        ...
        if args.key_file is not None:
            try:
                ...
                raise SmartBgtCliException(str(pe))
            except IOError as ioe:
                raise SmartBgtCliException(str(ioe))
        else:
            self._signer = crypto_factory.new_signer(
    ...
    def on_batch_committed(self, batch_id):
        ...
        if key is not None:
            if key.value < 1000000:
                txn = create_smart_bgt_transaction(
                    verb="inc",
                    name=key.name,
                    value=1,
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        SmartBgtState(name=name, url=url, value=0)
...
def do_workload(args):
    ...
    try:
        ...
        workload = SmartBgtWorkload(generator, args)

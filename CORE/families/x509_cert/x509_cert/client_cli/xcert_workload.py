...
from x509_cert.client_cli.workload.workload_generator import WorkloadGenerator
from x509_cert.client_cli.workload.dgt_workload import Workload
from x509_cert.client_cli.create_batch import create_xcert_transaction
from x509_cert.client_cli.create_batch import create_batch
from x509_cert.client_cli.exceptions import XcertCliException
from dgt_sdk.protobuf import batch_pb2

LOGGER = logging.getLogger(__name__)

XcertState = namedtuple('XcertState', ['name', 'url', 'value'])
...
class XcertWorkload(Workload):
    ...
    def __init__(self, delegate, args):
        super(XcertWorkload, self).__init__(delegate, args)
        ...
        if args.key_file is not None:
            try:
                ...
                raise XcertCliException(str(pe))
            except IOError as ioe:
                raise XcertCliException(str(ioe))
        else:
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        XcertState(name=name, url=url, value=0)
...
def do_workload(args):
    """
    Create WorkloadGenerator and XcertWorkload. Set Bgt workload in
    generator and run.
    """
    try:
        args.auth_info = _get_auth_info(args.auth_user, args.auth_password)
        generator = WorkloadGenerator(args)
        workload = XcertWorkload(generator, args)

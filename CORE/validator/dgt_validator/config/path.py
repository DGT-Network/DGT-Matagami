...
def get_default_path_config():
    """Returns the default PathConfig as calculated based on DGT_HOME
    (if set) and operating system.
    """
    if 'DGT_HOME' in os.environ or 'PEER_HOME' in os.environ:
        home_dir = os.environ['PEER_HOME'] if 'PEER_HOME' in os.environ else os.environ['DGT_HOME']
        return PathConfig(

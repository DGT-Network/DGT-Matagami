...
class BaseERC20Benchmark(BaseBenchmark):
    def __init__(self, num_blocks: int = 2, num_tx: int = 50) -> None:
        ...
        self.w3 = Web3()
        self.addr1 = Web3.to_checksum_address(FUNDED_ADDRESS)# toChecksumAddress(FUNDED_ADDRESS)
        self.addr2 = Web3.to_checksum_address(SECOND_ADDRESS) #toChecksumAddress(SECOND_ADDRESS)
    ...
    def _deploy_simple_token(self, chain: MiningChain, nonce: int = None) -> None:
        ...
        print("_deploy_simple_token")
        SimpleToken = self.w3.eth.contract(
            abi=self.contract_interface["abi"], bytecode=self.contract_interface["bin"]
        )
        print("_deploy_simple_token",SimpleToken)
        # Build transaction to deploy the contract
        w3_tx = SimpleToken.constructor().build_transaction(W3_TX_DEFAULTS)
        print("w3_tx",w3_tx)
        tx = new_transaction(
            vm=chain.get_vm(),
        ...
        def callback(receipt, computation) -> None:
            ...
            print("deployed_contract_address",self.deployed_contract_address)
            # Keep track of simple_token object
            self.simple_token = self.w3.eth.contract(
                address=Web3.to_checksum_address(#)toChecksumAddress(

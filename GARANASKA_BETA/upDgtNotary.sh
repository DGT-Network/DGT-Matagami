...
function upNotary {
  ...
  for node in $@;do
    ...
    case $node in
        1)
          export COMPOSE_PROJECT_NAME=1 LA=$LADDR C=c1   N=1 V=1 COMP=4104 VPORT=8220 CPORT=8221 SADDR=vault-n1:8220 ONBOT= ;docker-compose -f docker/docker-compose-notary-raft-dgt.yaml $mode
        ;;                                                        
        2)                                                        
          export COMPOSE_PROJECT_NAME=2 LA=$LADDR C=c1   N=1 V=2 COMP=4104 VPORT=8320 CPORT=8321 SADDR=vault-n1:8220 ONBOT=-bon;docker-compose -f docker/docker-compose-notary-raft-dgt.yaml $rmode
        ;;                                                                          
        3)                                                                          
          export COMPOSE_PROJECT_NAME=3 LA=$LADDR C=c1   N=1 V=3 COMP=4104 VPORT=8420 CPORT=8421 SADDR=vault-n1:8220 ONBOT= ;docker-compose -f docker/docker-compose-notary-raft-dgt.yaml $mode
        ;;                                                                          
        4)                                                                          
